 ...
 * the app with no profiles at all while every gate stayed green. */
{
   ...
   * which is the one condition this build exists for. */
  {
    ...
    if (aHtml && aSw) {
      ...
      /* ITS OWN ICON. The arcade shipped wearing the app's wave, so two apps
       * sat on the home screen under one icon and you could not tell which you
       * were opening. The files are generated by tools/make-icons.js; this
       * only checks it has not drifted back to borrowing the app's. */
      const aMan = read('juegos/manifest.json');
      ok(aMan.indexOf('../icons/') === -1,
         'juegos/manifest.json points at the app icons — the arcade needs its own, see tools/make-icons.js');
      ok(aHtml.indexOf('../apple-touch-icon.png') === -1,
         'juegos/index.html uses the app apple-touch-icon');
      ['icon-192.png', 'icon-512.png', 'icon-512-maskable.png', 'apple-touch-icon.png'].forEach(f => {
        ok(fs.existsSync(path.join(__dirname, '..', 'juegos/icons', f)),
           `juegos/icons/${f} is missing — run: node tools/make-icons.js`);
        ok(aSw.indexOf("'./icons/" + f + "'") !== -1,
           `juegos/sw.js does not cache icons/${f}, so the installed icon would not survive offline`);
      });
