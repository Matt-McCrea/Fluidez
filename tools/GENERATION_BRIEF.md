...
2. **Run both gates after every batch, and fix everything before continuing:**
   ```
   node tools/validate-content.js && node tools/test-checker.js && node tools/lint-spanish.js
   ```
   Done means all three print ✅. **Never fix a failure by weakening a validator
...
4. **Every verb you conjugate must exist in `data/verbs.js`.** Grep first.
...
Vocabulary headwords, collocations, level/theme/PCIC tags, and cloze answers
are produced mechanically from the spec and the conjugation engine. Writing
them by hand introduces errors the scripts cannot. If a task looks like
